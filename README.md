...
- **日程調整の自動化**: 調整さん(chouseisan.com)に日時候補の出欠表を作成してグループに投稿し、集計上位2日程でイベントを自動作成(かんたん=来月全日程/カスタム=カレンダーで日付と時刻を自由に選択)
- 日程別LINEグループの案内(招待リンクの投稿。グループに入った人がその日程の参加者)
- 自己紹介スライドの記入依頼
...
|---|---|---|
| グループの自動作成・招待 | ボットはグループを作成・招待**できない** | 運営者がグループを作りボットを招待。ボットはjoinイベントでグループを自動登録し、管理画面で日程に紐付ける。メンバーへは招待リンクをメイングループに投稿し、自分で参加してもらう |
...
- コントラクト層は `src/contracts/`(フォーム入力・テンプレート変数のzodスキーマ)

```
LINEグループ ──webhook──▶ /api/line/webhook ──▶ グループ登録
...
2. 環境変数を設定(`.env.example` 参照):
   ...
   (`CRON_SECRET` は `openssl rand -hex 32` などで生成したランダム文字列。任意の `DOCS_TOKEN` は「運営マニュアルの共有」、任意の `SLACK_WEBHOOK_URL` は「日程調整(調整さん連携)」参照)
...
36人メイングループ + 日程別グループ各18人・月1開催・1ボット構成だと**約250通(日程調整投稿込み)で枠を超過**します。そのため**2チャネルのグループ分散**を推奨します:
...
1. 出欠表を作成する(候補は「8/1(土) 20:00」のような日付+開始時刻。時刻は30分刻み)。**回答の締切日時**も必須で指定します
   - **「+ 来月の日程調整を開始」**(かんたん作成) — 来月の全日程×選んだ開始時刻(既定20:00)を候補に自動作成
   - **「+ カスタム日程調整を開始」** — カレンダーで候補日を自由に選択(月またぎ可)。開始時刻は共通値+日付ごとの個別変更が可能
   → どちらもURLをメイングループにLINE投稿します
2. メンバーは調整さんで ◯/△/× を入力(LINEログイン等は不要)
3. 締切日時を過ぎると、次の `/api/cron/tick`(最大5分後)で自動的に出欠表を集計(◯=1点・△=0.5点、同点は早い日付優先)し、上位2日程のイベントを候補の時刻で自動作成します(開始時刻は日程カードで変更可能)。締切前に早めに取り込みたい場合は、日程調整の一覧から**「結果を取り込んでイベント作成」**を手動で押すこともできます
4. 締切処理の結果(採用された2日程それぞれに◯・△を付けた人。回答が集まらなかった場合はその旨)は `SLACK_WEBHOOK_URL` を設定していればSlackに通知されます。未設定でも締切検知・自動取込自体は動きます(通知だけがオフになる)。SlackのIncoming Webhook URLは、対象チャンネルの「アプリを追加」→「Incoming Webhooks」から発行できます
